  ...
  <div class="app-container">
    ...
    <el-form :inline="true" :model="search" class="demo-form-inline">
      <el-form-item label="姓名">
        <el-input v-model="search.title" placeholder="查询姓名关键子" />
      </el-form-item>
      <el-form-item label="地址">
        <el-input v-model="search.sellPoint" placeholder="查询地址关键字" />
      </el-form-item>
      <el-form-item>
    ...
    <el-table :data="tableData" border style="width: 100%;">
      ...
      <el-table-column prop="title" label="商品标题" width="120" />
      ...
      <el-table-column prop="cid" label=" 所属类目" width="120" />
      <el-table-column label="状态" width="160">
        <template slot-scope="scope">
          <el-switch v-model="scope.row.status" active-text="上架" inactive-text="下架" :active-value="1" :inactive-value="2" @change="changeStatus(scope.row)" />
        </template>
    ...
    >
      <el-form ref="customer" :model="customer" label-width="80px">
        <el-form-item label="姓名">
          <el-input v-model="customer.title" />
        </el-form-item>
  ...
  methods: {
    ...
    // 商品上下价
      ...
      this.$confirm('是否上架?', '提示', {
        ...
        // 操作
        console.log('操作' + row.id + '数据，状态：' + row.status)
        // axios 执行后台的请求 进行删除
        upDateShop(row.id, row.status).then(res => {
          this.$message({
            message: '操作成功',
            type: 'success'
          })
